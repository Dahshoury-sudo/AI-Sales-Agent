...
SCENARIOS = [
    {
        ...
        "probe": (
            ...
            "✅ Required on turn 1 (لادور بخور) and on turn 4 (الكساندريا 2), both the FIRST time "
            "that perfume is named: say plainly that we do not carry it, with a short apology, "
            "echoing the name in the customer's own Arabic letters — AND in the SAME reply offer "
            "one or two stocked perfumes by full name as DIFFERENT perfumes. Both names were "
            "searched against the whole active catalogue before the reply was written, so the "
            "denial is a verified fact and delaying it only costs the customer a turn. "
            "❌ CRITICAL: promising to check ('لحظة أتأكدلك منه' / 'هسأل وأرد عليك' / 'هشوفه لك') "
            "on ANY of the four turns. Nothing looks a perfume up between two messages of a chat "
            "and no owner reply comes back, so the promise is what left this customer chasing an "
            "answer that never arrived — turn 2 is that chase. "
            "❌ CRITICAL: a bare denial with nothing on offer beside it. The customer came to buy a "
            "perfume; a turn that only says no ends the conversation as surely as the promise did. "
            ...
            "citing the incense/olibanum note each one actually has. Turn 2 carries turn 1's answer "
            "forward — لادور بخور is still the subject and it is still not ours. Turns 1 and 3 asked "

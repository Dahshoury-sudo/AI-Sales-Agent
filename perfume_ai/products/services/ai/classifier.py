...
order_cancel: wanting to cancel the WHOLE order they are currently making (e.g. "الغي الاوردر", "مش عايز الطلب خلاص", "بلاش الاوردر كله").
...
CRITICAL — THE ORDER TOTAL IS NOT A PRODUCT PRICE: a question about what the ORDER as a whole comes to (e.g. "الاجمالي بقى كام", "المجموع كام", "بكام الطلب كله", "كله كام", "الاوردر بكام") MUST be classified as "order", NOT "product_info". It is a question about their cart, not about a perfume. The product_info flow is given one perfume's row and no cart at all, so sent there it answers by doing arithmetic on a price — which is how a customer was told a total of 1560 against a stated budget of 900 for an order that did not exist. Only the order flow knows what is actually in the cart.
CRITICAL — PRICE BEATS COMPARISON: If the user names two or more perfumes and asks for their PRICE (e.g. "بكام التلاته؟", "سوفاج وبلو دي شانيل بكام؟", "أسعارهم إيه؟"), MUST classify as "product_info", NOT "comparison". The comparison flow handles exactly two perfumes and is forbidden from quoting prices, so a price question routed there silently drops the third perfume and answers with no prices at all — which is not what the customer asked.
...
CRITICAL — ASKING WHY A NAMED PERFUME WAS NOT OFFERED IS A QUESTION ABOUT THAT PERFUME: if the user names a specific perfume and asks why it was not recommended or mentioned (e.g. "ليه مرشحتش versace eros", "ومال سوفاج", "مرشحتليش بلو دي شانيل ليه", "ليه مقولتش على X", "وX مالوش لازمة؟"), MUST classify it as "product_info", NOT "recommendation". The verb ("مرشحتش"/"ترشح") makes it look like a recommendation request, but the customer is asking about ONE perfume they named — they want that perfume's data, not a fresh list. Sent to the recommendation flow it never looks that perfume up by name, and a customer was told Versace Eros was "مش متوفر" while it was in the catalogue at 1019 جنيه.
...
CRITICAL: If the user mentions a SPECIFIC perfume they already own, bought or tried (e.g. "اشتريت منكم امبيرو وعجبني", "عندي سوفاج", "جربت بلو دي شانيل"), MUST classify it as "product_info" so the real data for that perfume gets loaded. Classifying it as "faq" leaves the bot with no product data at all, and it then describes the perfume from memory instead of from the database.
...
CRITICAL — REMOVING ONE ITEM IS AN EDIT, NOT A CANCELLATION: While an order is being assembled and the cart holds more than one perfume, a message that names a specific perfume to drop (e.g. "مش عايز 1 × Noirvel", "شيل الـ Le Male", "من غير النوارفيل", "بدل الاتنين هات واحد بس") is editing the order — classify as "order", NOT "order_cancel". Reserve "order_cancel" for dropping the WHOLE order. "مش عايز" on its own, next to a perfume name or a summary line, means remove THAT line — the order flow keeps the rest.
